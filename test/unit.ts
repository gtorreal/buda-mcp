...
import { validateMarketId, validateCurrency } from "../src/validation.js";
import { flattenAmount, getLiquidityRating, aggregateTradesToCandles, parseEnvInt } from "../src/utils.js";
...
// ----------------------------------------------------------------
// a. Cache deduplication — fetcher called exactly once for concurrent requests
// ----------------------------------------------------------------

section("a. Cache in-flight deduplication");
...
// ----------------------------------------------------------------
// b. Input sanitization — malformed market IDs return isError:true
// ----------------------------------------------------------------

section("b. Input sanitization — validateMarketId");
...
// ----------------------------------------------------------------
// c. 429 retry — mock 429 then 200, assert fetch called twice
// ----------------------------------------------------------------

section("c. 429 Retry-After retry logic");
...
await test("throws BudaApiError with retryAfterMs when second attempt also returns 429", async () => {
  ...
  try {
    ...
    assert(threw, "should throw after both attempts return 429");
    assertEqual(retryAfterMs, 5000, "retryAfterMs should be 5000 (5s * 1000)");
  } finally {
    globalThis.fetch = savedFetch;
  }
});
...
await test("Retry-After header parsed as seconds (RFC 7231): '2' header = 2000ms wait", async () => {
  ...
  try {
    ...
    assertEqual(delays[0], 2000, "Retry-After: 2 should produce a 2000ms delay");
  } finally {
    globalThis.fetch = savedFetch;
...
// ----------------------------------------------------------------
// d. Numeric flattening — flattenAmount returns typed float, not string
// ----------------------------------------------------------------

section("d. Numeric flattening — flattenAmount");
...
// ----------------------------------------------------------------
// e. get_arbitrage_opportunities — discrepancy calculation
// ----------------------------------------------------------------

section("e. get_arbitrage_opportunities — discrepancy calculation");
...
// ----------------------------------------------------------------
// f. get_market_summary — liquidity_rating thresholds
// ----------------------------------------------------------------

section("f. get_market_summary — liquidity_rating thresholds");
...
// ----------------------------------------------------------------
// g. simulate_order — simulation outputs
// ----------------------------------------------------------------

section("g. simulate_order");
...
// ----------------------------------------------------------------
// h. calculate_position_size — position math
// ----------------------------------------------------------------

section("h. calculate_position_size");
...
// ----------------------------------------------------------------
// i. get_market_sentiment — scoring and labels
// ----------------------------------------------------------------

section("i. get_market_sentiment");
...
// ----------------------------------------------------------------
// j. get_technical_indicators — math and edge cases
// ----------------------------------------------------------------

section("j. get_technical_indicators");
...
// ----------------------------------------------------------------
// l. get_available_banks
// ----------------------------------------------------------------

section("l. get_available_banks");
...
// ----------------------------------------------------------------
// m. get_real_quotation
// ----------------------------------------------------------------

section("m. get_real_quotation");
...
await test("handleGetRealQuotation: INVALID_MARKET_ID without fetch", async () => {
  ...
  try {
    const client = new BudaClient("https://www.buda.com/api/v2");
    const result = await handleGetRealQuotation({ market_id: "BADINPUT", type: "Bid", amount: 1 }, client);
    assert(result.isError === true, "should be error");
...
await test("parseEnvInt: value below min → throws", () => {
  ...
  assert(threw, "value below min should throw");
});

await test("parseEnvInt: value above max → throws", () => {
  let threw = false;
  try { parseEnvInt("65536", 3000, 1, 65535, "PORT"); } catch { threw = true; }
  assert(threw, "value above max should throw");
});
...
// ----------------------------------------------------------------
// Security — Retry-After cap (max 30 s)
// ----------------------------------------------------------------

section("Retry-After cap — max 30 000 ms enforced");
...
await test("logAudit with different IPs in nested requestContext.run() calls uses innermost ip", async () => {
  ...
  try {
    ...
    assert(captured.length > 0, "logAudit should write to stderr");
    const parsed = JSON.parse(captured[captured.length - 1]) as { ip?: string };
    assertEqual(parsed.ip, "192.168.1.1", "innermost requestContext ip should take precedence");
  } finally {
    process.stderr.write = savedWrite;
  }
});
